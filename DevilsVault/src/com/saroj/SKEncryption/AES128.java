package com.saroj.SKEncryption;
...
public class AES128 {
	...
	String _initVector = "a4xF0@345#&SSmEK"; // AES128.generateRandomIV(16); //16 bytes = 128 bit;
	SecretKeySpec keySpec;
	IvParameterSpec ivSpec;
	
	public AES128(String _encryptionKey) throws NoSuchAlgorithmException, NoSuchPaddingException, UnsupportedEncodingException, 
																					InvalidAlgorithmParameterException, InvalidKeyException {
		// initialize the cipher with transformation AES/CBC/PKCS5Padding
		_cxEncryptor = Cipher.getInstance("AES/CBC/PKCS5Padding");
		_cxDecryptor = Cipher.getInstance("AES/CBC/PKCS5Padding");
		...
		keySpec = new SecretKeySpec(_key, "AES"); 
		ivSpec = new IvParameterSpec(_iv);
		
		_cxEncryptor.init(Cipher.ENCRYPT_MODE, keySpec, ivSpec);// Initialize this cipher instance
		_cxDecryptor.init(Cipher.DECRYPT_MODE, keySpec, ivSpec);// Initialize this ipher instance
	}
	 ...
	 */
	 	private String encryptDecrypt(String _inputText, EncryptMode _mode) throws UnsupportedEncodingException, 
		...
		if (_mode.equals(EncryptMode.ENCRYPT)) {
			...
			byte[] results = _cxEncryptor.doFinal(_inputText.getBytes("UTF-8")); // Finish
										// multi-part
										// transformation
										// (encryption)
		...
		if (_mode.equals(EncryptMode.DECRYPT)) {
			...
			byte[] decodedValue = Base64.decode(_inputText.getBytes(),
					Base64.DEFAULT);
			byte[] decryptedVal = _cxDecryptor.doFinal(decodedValue); 
	...
	public String encrypt(String _plainText) throws InvalidKeyException,
	 ...
	 * @throws BadPaddingException
	 */
	public String decrypt(String _encryptedText) throws InvalidKeyException, UnsupportedEncodingException,
...
//		cipher = Cipher.getInstance("AES/CBC/PKCS5Padding", "SunJCE");
		sKey = new SecretKeySpec(key.getBytes("US-ASCII"), "AES");
	}
		
	public String encrypt(String text) throws Exception  {
		cipher.init(Cipher.ENCRYPT_MODE, sKey, new IvParameterSpec(IV.getBytes("UTF-8")));
		return (new BASE64Encoder()).encodeBuffer(cipher.doFinal(text.getBytes("UTF-8")));
	}
	
	public String decrypt(String cipherText) throws Exception  {
		cipher.init(Cipher.DECRYPT_MODE, sKey, new IvParameterSpec(IV.getBytes("UTF-8")));
		return (new String(cipher.doFinal((new BASE64Decoder()).decodeBuffer(cipherText)), "UTF-8")).trim();
	}
		
	
	public static void main(String[] args) throws Exception {
		String s = "Hello there. How are you? Have a nice day.";
		AES128 aes = new AES128(new String(SecureRandom.getSeed(16)));
